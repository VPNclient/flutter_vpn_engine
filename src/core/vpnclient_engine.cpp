    ...
    if(config.proxy != ProxyMode::NONE) {
        switch(config.proxy) {
            case ProxyMode::VPNCLIENTDRIVER:
                client->set_proxy(std::make_unique<VPNclientDriverProxy>(config.proxy_config));
                break;
            case ProxyMode::TUN2SOCKS:
                client->set_proxy(std::make_unique<Tun2SocksProxy>(config.proxy_config));
                break;
            case ProxyMode::HEV_SOCKS5:
                client->set_proxy(std::make_unique<HevSocks5Proxy>(config.proxy_config));
                break;
